...
export default function Wallet() {
  ...
  const getTransactionIcon = (type) => {
    switch (type) {
      ...
      case "refund":
        return <RefreshCw className={`h-6 w-6 md:h-7 md:w-7 lg:h-8 lg:w-8 ${BRAND_THEME.tokens.wallet.refundText}`} />
      default:
        return null
    }
  }
  ...
  const getTransactionColor = (type) => {
    switch (type) {
      ...
      case "refund":
        return BRAND_THEME.tokens.wallet.refundText
      default:
  ...
  return (
    <AnimatedPage className={`min-h-screen ${BRAND_THEME.tokens.wallet.pageBackground}`}>
      <div className={`${BRAND_THEME.tokens.wallet.headerSurface} sticky top-0 z-10 border-b border-gray-100 dark:border-gray-800`}>
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 md:gap-8 lg:gap-10">
              <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-6 lg:gap-8 flex-1">
                <div className="relative flex-shrink-0">
                  <div className="w-20 h-20 md:w-24 md:h-24 lg:w-28 lg:h-28 xl:w-32 xl:h-32 rounded-xl md:rounded-2xl flex items-center justify-center shadow-lg transform rotate-[-5deg]" style={{ backgroundImage: BRAND_THEME.tokens.wallet.brandGradient }}>
                  ...
                  <div className="absolute inset-0 rounded-xl md:rounded-2xl transform rotate-[-5deg] translate-y-1 -z-10 opacity-25" style={{ backgroundColor: BRAND_THEME.tokens.wallet.brandShadow }} />
                </div>
              ...
              <div className="flex-shrink-0 w-full md:w-auto">
                <Button
                  className={`w-full md:w-auto md:min-w-[200px] lg:min-w-[240px] h-12 md:h-14 lg:h-16 ${BRAND_THEME.tokens.wallet.addMoneyButton} font-semibold text-sm md:text-base lg:text-lg rounded-xl transition-all duration-200 flex items-center justify-center gap-2`}
                    ...
                    return (
                      <button
                        ...
                        className={`px-4 md:px-5 lg:px-6 py-2 md:py-2.5 lg:py-3 rounded-lg md:rounded-xl text-xs md:text-sm lg:text-base font-medium whitespace-nowrap flex-shrink-0 transition-all ${
                          isSelected
                            ? BRAND_THEME.tokens.wallet.filterActive
